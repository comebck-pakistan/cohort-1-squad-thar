# Meeting 1

**Attendees:** Abdul Moiz, Ahtasham Shabbir, Musa Qamar, Muhammad Talha Rao, Saif ur Rehman

- First Meeting with Mentor Saif ur Rehman
- Team introductions and getting to know each other.
- Discussed the overall development approach.
- Recommended coding the solution instead of using low/no-code platforms.
- Prepare the PRD/BRD and Solution Design (SD) before starting development.
- Create sequence diagrams for all major flows (especially onboarding).
- Define all actors, user flows, and system interactions.
- Finalize the database choice and overall architecture.
- Decide between WhatsApp or App implementation (my recommendation is WhatsApp).
- Model for OCR, use TROCR open source or if you need to go towards a paid solution than go for Google Vision API
- Clearly define the value proposition and business value.
- Divide the workload among the four team members and start execution.
- If you need assistance with AI tools or how to use them effectively during development, we can discuss that anytime.
